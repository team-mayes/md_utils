...
INCOMP_F_LIST_PATH = os.path.join(CP2K_DATA_DIR, 'cp2k_force_list_incomp.txt')
F_LIST_PATH = os.path.join(CP2K_DATA_DIR, 'cp2k_force_list_good.txt')
...
class TestConvertCP2K(unittest.TestCase):
    ...
    def testMissingIncompleteFile(self):
        with capture_stderr(convert_cp2k_forces.main, ["-l", INCOMP_F_LIST_PATH]) as output:
            self.assertTrue("SUM OF ATOMIC FORCES" in output)
            self.assertTrue("Could not read file" in output)
        with capture_stdout(convert_cp2k_forces.main, ["-l", INCOMP_F_LIST_PATH]) as output:
    ...
    def testGoodInput(self):
        with capture_stdout(convert_cp2k_forces.main, ["-l", F_LIST_PATH]) as output:
