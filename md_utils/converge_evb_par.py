...
def min_convex_param(cfg, iteration_dict, tpl_dict, tpl_str):
    for key, step in iteration_dict.items():
        best_result = np.inf
        min_val = None
        mid_val = None
        max_val = None
        iter_num = 0
        while step > cfg[MIN_STEP][key]:
            ...
            print("trial val", key, trial_val, check_output([cfg[BASH_DRIVER], str(trial_val)]))
            trial_result = float(check_output([cfg[BASH_DRIVER], str(trial_val)]).strip())
            iter_num += 1
            if abs(trial_result - best_result) < cfg[CONV_CUTOFF]:
                print("Minimized '{}' within the specified convergence tolerance ({}). Best result: {}. "
                      "Program will continue.".format(key, cfg[CONV_CUTOFF], best_result))
                break
            ...
            # Now I should have run the script at least 3 times, with exactly one val as None
                ...
                if mid_val[1] < min_val[1] and mid_val[1] < max_val[1]:
                    step /= 2.0
                    if step <= cfg[MIN_STEP][key]:
                        print("Minimized '{}' within specified tolerance (min step size) of {}. Best result: {}. "
                              "Program will continue.".format(key, cfg[MIN_STEP][key], best_result))
...
def main(argv=None):
    ...
    try:
        ...
        iteration_dict = cfg[MAX_STEP]
        num_params_to_min = len(iteration_dict)
        if num_params_to_min == 0:
            ...
            if cfg[PAR_COPY_NAME] is not None:
                copy_par_file(cfg, tpl_dict)
            trial_val = tpl_dict.values()[0]
            trial_result = float(check_output([cfg[BASH_DRIVER], str(trial_val)]).strip())
            print("Result without optimizing parameters: {}".format(trial_result))
        else:
            min_convex_param(cfg, iteration_dict, tpl_dict, tpl_str)
